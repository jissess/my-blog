...
layui.define(['element', 'form','laypage','jquery','laytpl'],function(exports){
  ...
  if ($("#test1").size() > 0) {
    ...
    laypage.render({
      ...
      ,jump: function(obj, first){
        ...
        $.get("/message/query",{pageno:obj.curr,pagesize:obj.limit}, function (res) {
          if (res.code == 0) {
            var datas = res.data;
            var html = "";
  ...
  function praise() {
    if (!($(this).hasClass("layblog-this"))) {
      ...
      var that = this;
      $.get("/praise/" + type + "/" + key, function (data) {
        if (data.code === 0) {
          that.text = '已赞';
  ...
  $('#item-btn').on('click', function(){
    ...
    $.post("/message/new", {content:content}, function (res) {
      if (res.code === 0) {
        var html = drawMessage(res.data);
        var $html = $(html);
        $html.find(".like").on("click", praise);
